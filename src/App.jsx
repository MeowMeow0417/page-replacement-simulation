...
// Generate a random page request pattern with some locality
...
// FIFO Page Replacement Algorithm: builds step-by-step simulation
...
// LRU Page Replacement Algorithm: builds step-by-step simulation
...
// OPT (Optimal) Page Replacement Algorithm: builds step-by-step simulation
...
// Controls for simulation: play, pause, step, reset, and speed
...
// Shows statistics: hits, faults, total requests
...
function CacheSim() {
  // State for simulation parameters and progress
  ...
  // Generate requests and build steps for the selected algorithm
  const runSimulation = () => {
  ...
  // Calculate statistics for the current simulation step
  ...
  // Handle play/pause simulation with interval
  useEffect(() => {
    if (isPlaying) {
    ...
    <section className="flex flex-col p-6 w-full mx-auto h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex flex-col gap-6 w-full md:max-w-sm">
          ...
          {/* Aside Controls: algorithm, cache size, request count, generate */}
          ...
          {/* Simulation Controls: only show if simulation has started */}
          {steps.length > 0 && (
            <SimulationControls
              isPlaying={isPlaying}
        ...
        <div className="flex flex-col flex-1 gap-6 sm:pb-24">
          ...
          {/* Page Reference String: show all generated requests */}
          {requests.length > 0 ? (
            <Card className="w-full">
              <CardHeader>
          ...
          {/* Current Request Visualizer: show current request and cache state */}
          {steps.length > 0 && (
          ...
          {/* Statistics Dashboard: show stats for current simulation */}
          {steps.length > 0 && (
            <StatsDashboard
